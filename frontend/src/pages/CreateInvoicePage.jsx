...
import { Container, Row, Col, Card, Button, Table, Form, InputGroup, Alert, Badge, Spinner, ListGroup } from 'react-bootstrap';
import {
    FaFileInvoiceDollar, FaCheck, FaTimes, FaUser, FaCar, FaPlus, FaInfoCircle
} from 'react-icons/fa';
import {
    initialJobSheets,
    addInvoice,
...
const CreateInvoicePage = () => {
    ...
    const handleFinalizeInvoice = () => {
        ...
        if (window.confirm("Finalize this Invoice? It will be saved and assigned an Invoice Number.")) {
            ...
            setTimeout(() => {
                try {
                    const savedInvoice = addInvoice(finalInvoiceData);
                    setIsFinalizing(false);
                    alert(`Invoice ${savedInvoice.invoiceNumber} Created Successfully!`);
                    navigate(`/invoice/${savedInvoice.id}/view`);
                } catch (err) {
    ...
    return (
        <Container fluid className="py-4 ">
            ...
            <div className="page-header-row">
                <h2 className="page-title-active mb-40">
                    <FaFileInvoiceDollar />
                    Create Invoice
                </h2>
            </div>
